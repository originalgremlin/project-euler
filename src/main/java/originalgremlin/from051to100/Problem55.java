 ...
 * theoretical nature of these numbers, and for the purpose of this problem, we shall assume that a number is
...
public class Problem55 {
    public static void main (String[] args) {
        long count = LongStream.range(1, 10000)
                .filter(Problem55::isLychrelNumber)
                .count();
        System.out.println(count);
    }

    private static boolean isLychrelNumber (long test) {
        BigInteger fwd = new BigInteger(Long.toString(test));
        BigInteger rev;
        for (int i = 0; i < 50; i++) {
            rev = new BigInteger(new StringBuilder(fwd.toString()).reverse().toString());
            fwd = fwd.add(rev);
            if (Euler.isPalindrome(fwd.toString())) {
                return false;
            }
        }
        return true;
